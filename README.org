...
양방향 통신을 사용해서 Effects Block의 on/off변화를 미디 캡틴과 동기화
...
*** 3. 프로젝트 파일 복사
이 저장소의 다음 파일들을 =MIDICAPTAIN= 드라이브 루트에 덮어쓴다.

| 파일         | 용도                                        |
|--------------+---------------------------------------------|
| =code.py=    | 메인 펌웨어 (FM3/FM9 양방향 SysEx 컨트롤러) |
| =boot.py=    | 부팅 모드 제어 (Switch 1 → USB 드라이브)    |

필요 라이브러리(=lib/= 폴더): =adafruit_midi=, =adafruit_st7789=, =adafruit_display_text=
- 순정 펌웨어의 =lib/= 폴더에 이미 포함되어 있으므로 보통 그대로 사용하면 된다
- 없거나 손상된 경우 아래 [[*CircuitPython libs][CircuitPython libs]] 절 참고하여 bundle 7.x에서 추출해 복사
...
- 개발/디버깅 :: [[file:doc/mu.org][Mu Editor 사용법]] 참고. autoreload가 기본 활성화되어 있어 =code.py= 저장 즉시 반영된다 (끄려면 =boot.py= 의 =supervisor.disable_autoreload()= 주석 해제)
...
*** 풋스위치 10개 GPIO 맵
  ...
  | Encoder B         | GP3  |
  |-                  |      |
** [[file:doc/user_manual.org][사용자 매뉴얼]]
