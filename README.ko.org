...
*Paint Audio MIDI Captain* + *Fractal Audio FM3* 양방향 펌웨어
...
*** 프로그래머블 미디 컨트롤러(예: Morningstar MC6 Pro)로는 안 되나?
프로그래머블 컨트롤러는 임의 SysEx를 *송신*할 수 있고, 수신 메시지가 특정 바이트열과 정확히 일치하면
액션을 실행하는 기능이 있어 수작업으로 제한적인 동기화는 가능합니다 — 예: scene 변경 응답 8종을
각각 프리셋 토글에 매핑. 하지만 다음은 할 수 없습니다:
- 가변 길이 응답을 바이트 단위로 해석 (FM3 status dump는 한 메시지 안에 3바이트 패킷 수십 개)
- 타이머 폴링 ("0.15초마다 FM3에 조회" 루프 없음)
- 응답에서 문자열(프리셋/씬 이름)을 꺼내 화면에 표시
따라서 이펙트 블록/채널/이름까지 완전 동기화하려면 *자기 코드가 도는 컨트롤러*가 필요하고,
이 펌웨어가 MIDI Captain(RP2040 + CircuitPython)에 그 역할을 부여합니다.
...
- 설정이 저장되지 않음 / 평상시 부팅 후에도 =config.json= 이 생기지 않음 :: 기기 FAT 파일시스템 손상 가능성 (파일 대량 삭제나 안전하지 않은 추출 뒤에 흔함). 기기에서 포맷 후 재설치: Update Mode 진입 → 시리얼 콘솔(Mu 등) 열기 → Ctrl-C로 REPL → =import storage; storage.erase_filesystem()= 실행 → 기기가 포맷 후 빈 =CIRCUITPY= 드라이브로 재부팅 → 3단계대로 =code.py=, =boot.py=, =lib/= 다시 복사. 사용자 설정은 사라지므로 가능하면 =config.json= 을 먼저 백업.
